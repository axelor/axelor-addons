...
public class BatchGSuiteTaskSyncService extends AbstractBatch {
  ...
  @Inject GSuiteTaskImportService gSuiteTaskImportService;
  @Inject GSuiteTaskExportService gSuiteTaskExportService;
    ...
    for (GoogleAccount account : accountSet) {
      try {
        if (batch.getgSuiteBatch().getTypeSelect() == GSuiteBatchRepository.TYPE_SELECT_IMPORT) {
          gSuiteTaskImportService.sync(account);
        } else {
          gSuiteTaskExportService.sync(account);
        }
        incrementDone();
