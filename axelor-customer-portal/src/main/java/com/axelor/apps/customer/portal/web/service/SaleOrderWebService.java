...
public class SaleOrderWebService extends AbstractWebService {
  ...
  @POST
  @Path("/{id}/cancel")
  public PortalRestResponse cancel(@PathParam("id") Long id) throws AxelorException {

    SaleOrder order = findById(id);

    Beans.get(JpaSecurity.class).check(AccessType.WRITE, SaleOrder.class, order.getId());

    Beans.get(SaleOrderWorkflowService.class)
        .cancelSaleOrder(order, null, I18n.get("Cancelled from Portal"));
  ...
  @POST
  @Path("/{id}/confirm")
  public PortalRestResponse confirm(@PathParam("id") Long id) throws AxelorException {
    SaleOrder order = findById(id);
    ...
    return new PortalRestResponse().success();
  }

  private SaleOrder findById(Long id) throws AxelorException {
    final String filter = "self.clientPartner = :partner AND self.id = :id";
    Map<String, Object> params = ImmutableMap.of("partner", getPartner(), "id", id);
