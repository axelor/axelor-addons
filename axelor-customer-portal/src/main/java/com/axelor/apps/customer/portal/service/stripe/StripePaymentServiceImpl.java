 ...
 */
package com.axelor.apps.customer.portal.service.stripe;
...
public class StripePaymentServiceImpl implements StripePaymentService {
  ...
  @Override
  public Customer updateCustomer(Partner partner) throws StripeException, AxelorException {

    if (StringUtils.isBlank(partner.getStripeCustomerId())) {
      return null;
    }

    RequestOptions requestOptions =
        RequestOptions.builder().setApiKey(appRepo.all().fetchOne().getStripeSecretKey()).build();
    Address defaultAddress = partnerService.getDefaultAddress(partner);
    ...
    Customer customer = Customer.retrieve(partner.getStripeCustomerId(), requestOptions);

    Map<String, Object> address = new HashMap<>();
    address.put("line1", defaultAddress.getAddressL4());
    address.put(
        "city", defaultAddress.getCity() != null ? defaultAddress.getCity().getName() : null);
    address.put(
        "country",
        defaultAddress.getAddressL7Country() != null
            ? defaultAddress.getAddressL7Country().getAlpha2Code()
            : null);
    address.put("line2", defaultAddress.getAddressL2());
    address.put("postal_code", defaultAddress.getZip());

    Map<String, Object> params = new HashMap<>();
    params.put("name", partner.getFullName());
    params.put(
        "email",
        Optional.ofNullable(partner.getEmailAddress()).map(EmailAddress::getAddress).orElse(null));
    params.put("phone", partner.getMobilePhone());
  ...
  public Charge checkout(Invoice invoice, Customer customer, String cardId, BigDecimal payAmount)
    ...
    if (charge != null) {
      ...
      Beans.get(InvoicePaymentToolService.class).updateAmountPaid(invoice);
      Beans.get(InvoicePaymentValidateService.class).validate(invoicePayment, true);
