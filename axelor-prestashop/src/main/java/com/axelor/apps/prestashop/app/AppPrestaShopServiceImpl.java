...
public class AppPrestaShopServiceImpl implements AppPrestaShopService {
  ...
  private static final HashSet<PrestashopResourceType> REQUIRED_XLINKS =
      Sets.newHashSet(
  ...
  private static final Set<PrestashopResourceType> DELETABLE_XLINKS =
      Sets.newHashSet(
          PrestashopResourceType.DELIVERIES,
          PrestashopResourceType.ORDER_INVOICES,
          PrestashopResourceType.ORDER_PAYMENTS);
  ...
  public void checkAccess(
    ...
    try {
      ...
      Set<PrestashopResourceType> requiredEntries =
      ...
      for (XlinkEntry entry : api.getXlinkEntries()) {
        if (requiredEntries.remove(entry.getEntryType())) {
          ...
          info.add(
              String.format(
                  I18n.get("Extra permission for %s is uneeded"), entry.getEntryType().getLabel()));
        }
      }
        ...
        errors.add(
            String.format(
                I18n.get(
                    ...
                    "Missing access rights for entities %s", requiredEntries.size()),
                sb.toString()));
      }
