...
public class ExportOrderServiceImpl implements ExportOrderService {
  ...
  public void exportOrder(AppPrestashop appConfig, Writer logBuffer)
    ...
    logBuffer.write(
        String.format("%n=== END OF ORDERS EXPORT, done: %d, errors: %d ===%n", done, errors));
  }
  ...
  private void exportLines(
      ...
      final Integer remoteInvoiceId,
      final Writer logBuffer)
      throws PrestaShopWebserviceException, IOException {
    List<PrestashopOrderRowDetails> remoteRows =
        ws.fetch(
            PrestashopResourceType.ORDER_DETAILS,
            Collections.singletonMap("id_order", order.getPrestaShopId().toString()));
    ...
    for (SaleOrderLine localRow : lines) {
      logBuffer.write(
          String.format(
              "\tExporting line #%d (%s) ‑ ", localRow.getId(), localRow.getProductName()));
      PrestashopOrderRowDetails remoteRow;
      if (localRow.getPrestaShopId() == null
          || remoteRowsById.containsKey(localRow.getPrestaShopId()) == false) {
        if (localRow.getPrestaShopId() != null) {
          logBuffer.write(
              String.format(
                  "[WARNING] no row with id %d found remotely for this order, creating a new one",
                  localRow.getId()));
        }
      ...
      remoteRow.setProductId(
          localRow
              .getProduct()
              .getPrestaShopId()); // Ensured to be not null by preliminary filtering
      remoteRow.setProductName(localRow.getProductName());
      remoteRow.setProductQuantity(localRow.getQty().intValue());
      remoteRow.setUnitPriceTaxExcluded(
          localRow
              .getExTaxTotal()
              .divide(localRow.getQty())
              .setScale(appConfig.getExportPriceScale(), RoundingMode.HALF_UP));
      remoteRow.setUnitPriceTaxIncluded(
          localRow
              .getInTaxTotal()
              .divide(localRow.getQty())
              .setScale(appConfig.getExportPriceScale(), RoundingMode.HALF_UP));
      remoteRow.setProductPrice(
          localRow.getPrice().setScale(appConfig.getExportPriceScale(), RoundingMode.HALF_UP));
      if (localRow.getDiscountTypeSelect() != null) {
        switch (localRow.getDiscountTypeSelect()) {
          case PriceListLineRepository.AMOUNT_TYPE_PERCENT:
            remoteRow.setDiscountPercent(
                localRow
                    .getDiscountAmount()
                    .setScale(appConfig.getExportPriceScale(), RoundingMode.HALF_UP));
            break;
          case PriceListLineRepository.AMOUNT_TYPE_FIXED:
            remoteRow.setDiscountAmount(
                localRow
                    .getDiscountAmount()
                    .setScale(appConfig.getExportPriceScale(), RoundingMode.HALF_UP));
            break;
      ...
      if (localRow.getProduct().getIsShippingCostsProduct() == Boolean.TRUE) {
        remoteRow.setTotalShippingPriceTaxExcluded(
            localRow
                .getExTaxTotal()
                .setScale(appConfig.getExportPriceScale(), RoundingMode.HALF_UP));
        remoteRow.setTotalShippingPriceTaxExcluded(
        ...
        remoteRow.setTotalPriceTaxExcluded(
            localRow
                .getExTaxTotal()
                .setScale(appConfig.getExportPriceScale(), RoundingMode.HALF_UP));
        remoteRow.setTotalPriceTaxIncluded(
            localRow
                .getInTaxTotal()
                .setScale(appConfig.getExportPriceScale(), RoundingMode.HALF_UP));
      }
  ...
  private List<PrestashopOrderPayment> getPayments(
      final AppPrestashop appConfig, final SaleOrder order, final String prestashopReference) {
    ...
    for (AdvancePayment p : order.getAdvancePaymentList()) {
      if (p.getStatusSelect() == AdvancePaymentRepository.STATUS_VALIDATED) {
        ...
        try {
          payment.setConversionRate(
              currencyService.getCurrencyConversionRate(
                  order.getCurrency(),
                  appConfig.getPrestaShopCurrency(),
                  p.getAdvancePaymentDate()));
        } catch (AxelorException e) {
      ...
      for (InvoicePayment p : invoice.getInvoicePaymentList()) {
        if (p.getStatusSelect() == InvoicePaymentRepository.STATUS_VALIDATED) {
          ...
          try {
            payment.setConversionRate(
                currencyService.getCurrencyConversionRate(
                    order.getCurrency(), appConfig.getPrestaShopCurrency(), p.getPaymentDate()));
          } catch (AxelorException e) {
            log.warn("Unable to get payment conversion rate, using 1.0");
            payment.setConversionRate(BigDecimal.ONE);
          }
   ...
   * @param localOrder Local copy of the currently being processed order (fetched from db)
   * @param remoteOrder Remote copy of the order (fetched through Prestashop's WS). Some of its
   *     properties will be updated depending on created/updated entities.
   * @return The id of the invoice bound to the order, or <code>null</code> if no invoice was
   *     generated
   */
  private Integer generateInvoicingEntities(
      final AppPrestashop appConfig,
      final PSWebServiceClient ws,
      ...
      final Writer logBuffer)
      throws PrestaShopWebserviceException, IOException {
    final List<PrestashopOrderPayment> existingPayments =
        ws.fetch(
            PrestashopResourceType.ORDER_PAYMENTS,
            Collections.singletonMap("order_reference", remoteOrder.getReference()));
    PrestashopOrderInvoice remoteInvoice =
        ws.fetchOne(
            PrestashopResourceType.ORDER_INVOICES,
            Collections.singletonMap("id_order", remoteOrder.getId().toString()));
    PrestashopDelivery remoteDelivery =
        remoteOrder.getDeliveryNumber() != null
            ? ws.fetchOne(
                PrestashopResourceType.DELIVERIES,
                Collections.singletonMap("id", remoteOrder.getDeliveryNumber().toString()))
            : null;
    ...
    if (remoteInvoice == null) {
      ...
      remoteInvoice.setShopAddress(
          localOrder.getCompany().getName()
              + "\n"
    ...
    if (remoteInvoice.getTotalPaidTaxExcluded().compareTo(localOrder.getAmountInvoiced()) != 0) {
      ...
      remoteInvoice.setTotalPaidTaxIncluded(
          localOrder
              .getInTaxTotal()
              .setScale(appConfig.getExportPriceScale(), RoundingMode.HALF_UP));
      remoteInvoice.setTotalPaidTaxExcluded(
          localOrder
              .getExTaxTotal()
              .setScale(appConfig.getExportPriceScale(), RoundingMode.HALF_UP));
    ...
    final List<PrestashopOrderPayment> neededPayments =
        getPayments(appConfig, localOrder, remoteOrder.getReference());
    for (ListIterator<PrestashopOrderPayment> it = neededPayments.listIterator(); it.hasNext(); ) {
      ...
      for (ListIterator<PrestashopOrderPayment> it2 = existingPayments.listIterator();
          it2.hasNext(); ) {
        PrestashopOrderPayment remotePayment = it2.next();
        if (remotePayment.getAmount().compareTo(localPayment.getAmount())
            == 0 /* We might have checked for addDate too but… PrestaShop ignores it when saving and sets it to now() */) {
          it2.remove();
          it.remove();
          break;
        }
    ...
    // So neededPayments now contains payments without remote counterpart, existingPayments contains
    // remote payments with no local counterPart
    for (PrestashopOrderPayment payment : existingPayments) {
      if (log.isDebugEnabled()) {
    ...
    if (localOrder.getDeliveryState() == SaleOrderRepository.DELIVERY_STATE_DELIVERED) {
      if (remoteDelivery == null) {
        ...
        if (localOrder.getDeliveryAddress().getAddressL7Country() != null
            && localOrder.getDeliveryAddress().getAddressL7Country().getPrestaShopZoneId()
                != null) {
          remoteDelivery.setZoneId(
              localOrder.getDeliveryAddress().getAddressL7Country().getPrestaShopZoneId());
        } else {
        ...
        for (StockMove move : localOrder.getStockMoveList()) {
          if (deliveryDate == null
              || (move.getRealDate() != null && deliveryDate.isBefore(move.getRealDate()))) {
      ...
      if (remoteDelivery.getId() == null
          || remoteDelivery.getPrice().compareTo(remoteOrder.getTotalShippingTaxIncluded()) != 0) {
