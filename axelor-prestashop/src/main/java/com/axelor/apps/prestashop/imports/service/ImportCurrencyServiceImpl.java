...
public class ImportCurrencyServiceImpl implements ImportCurrencyService {
  ...
  public void importCurrency(AppPrestashop appConfig, ZonedDateTime endDate, Writer logBuffer)
      ...
      if (appConfig.getPrestaShopMasterForCurrencies() || localCurrency.getId() == null) {
        ...
        try {
          currentRate =
              currencyService.getCurrencyConversionRate(
                  localCurrency, appConfig.getPrestaShopCurrency(), LocalDate.now());
        ...
        if (remoteCurrency.getConversionRate() != null
          ...
          currencyConversionService.createCurrencyConversionLine(
              ...
              LocalDate.now(),
              remoteCurrency.getConversionRate(),
              appBaseService.getAppBase(),
              currencyConversionService.getVariations(
                  remoteCurrency.getConversionRate(), currentRate));
        }
      } else {
        logBuffer.write(
            " – local currency exists and PrestaShop isn't master for currencies, leaving untouched");
      }
    ...
    logBuffer.write(
        String.format("%n=== END OF CURRENCIES IMPORT, done: %d, errors: %d ===%n", done, errors));
  }
}
